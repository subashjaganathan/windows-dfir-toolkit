...
foreach ($Vol in $Volumes) {
    ...
    # -- Copy $MFT using raw volume read via cmd/robocopy trick ----------------
    Write-Host "  [*] Copying `$MFT from $Drive..." -ForegroundColor Cyan
    $MFTDest = "$OutDir\${SafeName}_MFT"
    try {
        # Method 1: Use built-in RawCopy via .NET raw stream
        $VolPath = "\\.\$($Vol.DriveLetter):"
        $MFTStream = [System.IO.File]::Open($VolPath, 'Open', 'Read', 'ReadWrite')
        # Just verify access - actual MFT copy needs specialized tool
        $MFTStream.Close()

        # Use esentutl for volume shadow or direct file copy
        # Try Volume Shadow Copy method first
        $VSSResult = (vssadmin list shadows /for=$Drive 2>&1) -join " "
        $HasVSS = $VSSResult -match "Shadow Copy Volume"

        if ($HasVSS) {
            $ShadowPath = if ($VSSResult -match "Shadow Copy Volume Name:\s*(\S+)") { $Matches[1] } else { $null }
            if ($ShadowPath) {
                ...
                Write-Log "MFT copied via VSS: $Drive -> $MFTDest"
            }
        }

        if (-not $VolResult.MFTCopied) {
            # Method 2: Direct raw file access via CreateFile
...
public class RawCopy {
    ...
    public static extern IntPtr CreateFile(string lpFileName, uint dwAccess,
        uint dwShare, IntPtr lpSecurity, uint dwCreation, uint dwFlags, IntPtr hTemplate);
...
"@ -ErrorAction SilentlyContinue
            ...
            # Record metadata even if copy fails
            $MFTInfo = Get-Item "$Drive\`$MFT" -Force -ErrorAction SilentlyContinue
            if ($MFTInfo) {
                $VolResult.MFTSizeBytes = $MFTInfo.Length
                $VolResult.MFTPath = "Metadata only - use FTK/Autopsy/MFTECmd for full extraction"
            }
            Write-Log "MFT direct access recorded for $Drive (use offline tool for full copy)"
        }
    } catch {
        $VolResult.Error = "MFT: $_"
        Write-Log "MFT copy failed for $Drive : $_" "WARN"
    }
