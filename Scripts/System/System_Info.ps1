...
# NTP
$W32 = (& "$env:SystemRoot\System32\w32tm.exe" /query /status 2>&1) -join "`n"
...
# Logged-on users - multi-method for all Windows editions
$LoggedOn = @()
try {
    $QueryExe = "$env:SystemRoot\System32\query.exe"
    if (Test-Path $QueryExe) {
        $RawOutput = & $QueryExe user 2>$null
        $LoggedOn = @($RawOutput | Select-Object -Skip 1 | ForEach-Object {
...
# Fallback: use CIM if query.exe not available (Server Core, LTSC)
if ($LoggedOn.Count -eq 0) {
    try {
        $LoggedOn = @(Get-CimInstance Win32_LoggedOnUser -ErrorAction SilentlyContinue |
            ...
            ForEach-Object {
                $Parts = $_ -split '"'
                if ($Parts.Count -ge 4) {
                    [PSCustomObject]@{ Username=$Parts[3]; Session="N/A"; ID="N/A"; State="Active"; IdleTime="N/A" }
                }
