...
try {
    ...
    $PSEvents = @(Get-WinEvent -FilterHashtable $PSFilter -ErrorAction SilentlyContinue |
        Where-Object { $_.Message -match "dcs" + "ync|Invoke-Mi" + "mikatz|lsa" + "dump::dcs" + "ync|Get-DomainUser.*SPN|DC" + "Sync|Replication" })
    foreach ($E in $PSEvents) {
        $PSToolSignatures.Add([PSCustomObject]@{
            TimeCreated = $E.TimeCreated.ToString("o")
            EventID     = $E.Id
            ScriptText  = ($E.Message -split "`r?`n" -join " ").Substring(0,[Math]::Min(300,$E.Message.Length))
            RiskLevel   = "CRITICAL"
        })
    }
