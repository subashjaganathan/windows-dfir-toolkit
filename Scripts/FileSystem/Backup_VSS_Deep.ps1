...
try {
    ...
    foreach ($E in $VSSEvents) {
        $VSSDeletionEvents.Add([PSCustomObject]@{
            ...
            EventID     = $E.Id
            Message     = ($E.Message -split [Environment]::NewLine -join " " -replace "`n"," ")
...
try {
    ...
    foreach ($E in $BackupEvents) {
        $BackupHistory.Add([PSCustomObject]@{
            ...
            Message     = (($E.Message -split [Environment]::NewLine -join " " -replace "`n"," ")).Substring(0,[Math]::Min(200,$E.Message.Length))
        })
    }
