...
# NOTE: Add AV exclusion before running:
# Add-MpPreference -ExclusionPath $PSScriptRoot
<#
.SYNOPSIS
    Collects App usage database, PowerShell history, and execution history artifacts.
    On Server OS, adds Windows Server-specific execution evidence.

.DESCRIPTION
    Workstation (Win10/11):
      - App usage DB copy (30-60 days of app/network usage)
...
.FORENSIC_SAFETY
    Read-only - App usage DB copied via esentutl
...
$OutDir    = "$BasePath\AppUsage_History_${Hostname}_${Timestamp}"
New-Item -ItemType Directory -Path $OutDir -Force | Out-Null
$LogFile   = "$BasePath\AppUsage_History_Execution.log"
$JsonFile  = "$BasePath\AppUsage_History_${Hostname}_${Timestamp}.json"

function Write-Log { param([string]$M,[string]$L="INFO") Add-Content $LogFile "$(Get-Date -Format o) [$L] :: $M" }
Write-Log "AppUsage/History collection started | Case: $CaseNum"
...
Write-Host "[*] Locating App usage database..." -ForegroundColor Cyan
...
if (Test-Path $SRUMPath) {
    ...
    $SRUMInfo = [PSCustomObject]@{ Collected=$false; Note="App usage database not found at $SRUMPath" }
...
Get-ChildItem "C:\Users" -Directory -ErrorAction SilentlyContinue | ForEach-Object {
    ...
    if (Test-Path $HistPath) {
        $Lines    = @(Get-Content $HistPath -ErrorAction SilentlyContinue -TotalCount 50000)
        $CopyDest = "$OutDir\PSHistory_$($_.Name).txt"
        Copy-Item $HistPath $CopyDest -Force -ErrorAction SilentlyContinue
        # Build one combined regex for efficiency instead of nested loops
        $SuspRegex = ($SuspPatterns | ForEach-Object { [regex]::Escape($_) }) -join "|"
        $SuspLines = @($Lines | Where-Object { $_ -match $SuspRegex })
        # Full history already copied to disk; only keep suspicious + count in JSON to save memory
        $PSHistories.Add([PSCustomObject]@{
            ...
            Note               = "Full command history preserved in CopiedTo file"
        })
        $Lines = $null
...
if ($IsServer) {
    ...
    try {
        $TEvents = Get-WinEvent -MaxEvents 1000 -FilterHashtable @{
    ...
    try {
        $WREvents = Get-WinEvent -MaxEvents 1000 -FilterHashtable @{
    ...
    try {
        $SvcEvents = Get-WinEvent -MaxEvents 1000 -FilterHashtable @{
            LogName   = "System"
            Id        = @(7045, 7036, 7040)
            StartTime = $StartTime
        } -ErrorAction SilentlyContinue
    ...
    try {
        $BITSEvents = Get-WinEvent -MaxEvents 1000 -FilterHashtable @{
